...
export function useReportsData() {
  ...
  async function fetchStockByCategory() {
    try {
      const { data: categoryData } = await supabase.from('inventory').select(`
          quantity,
          variant:product_variants(
            product:products(
              price,
      ...
      ;(categoryData || []).forEach((item: any) => {
        const categoryName =
          item.variant?.product?.categories?.name || 'Uncategorized'
        const price = item.variant?.product?.price || 0
      ...
      const categoryStockData: Array<CategoryStock> = Object.entries(
        categoryTotals,
      ).map(([name, data]) => ({
        category_name: name,
        total_quantity: data.quantity,
  ...
  async function fetchInventoryHealth() {
    try {
      ...
      const healthItems: Array<InventoryHealthItem> = (data || []).map(
        (item: any) => {
  ...
  async function fetchMovementsData() {
    try {
      ...
      const movements: Array<StockMovementItem> = (data || [])
        .map((item: any) => {
        ...
        .filter(Boolean) as Array<StockMovementItem>

      setStockMovements(movements)
      setMovementSummary({
        stock_in: stockIn,
        stock_out: stockOut,
        transfers: Math.floor(transfers),
      })
    } catch (error) {
  ...
  const totalInventoryValue = branchStock.reduce(
